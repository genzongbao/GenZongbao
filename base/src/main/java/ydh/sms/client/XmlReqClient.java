...
public class XmlReqClient extends AbsRestClient {
	...
	public String templateSMS(String templateId, String to, String param) {
		...
		try {
			...
			HttpResponse response=post("application/xml", sid, authToken, timestamp, url, httpclient, encryptUtil, body);
			HttpEntity entity = response.getEntity();
			if (entity != null) {
				result = EntityUtils.toString(entity, "UTF-8");
			}
	...
	@Override
	public String templateSMSCall(String templateId, String to, String param) {
		String result = "";
		DefaultHttpClient httpclient=getDefaultHttpClient();
		try {
			//MD5加密
